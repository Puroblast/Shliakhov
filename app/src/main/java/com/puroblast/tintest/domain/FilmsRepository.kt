...
class FilmsRepository(
    ...
    fun observeFilms(query: String, filmFilter: FilmFilter): Flow<List<Film>> {
        return filmsDao.getFavouriteFilms().mapLatest { favouriteFilms ->
            getTopFilms().map {film ->
                if (film in favouriteFilms) film.copy(isFavourite = true) else film.copy(isFavourite = false)
            }.filter {
                when (filmFilter) {
    ...
    suspend fun getFilm(id: String): Film {
        val films = memoryStorage.getFilms()?.filter { it.filmId == id.toInt() && it.description != null}
    ...
    private suspend fun getTopFilms(): List<Film> {
        return memoryStorage.getFilms() ?: filmsApi.getTopFilms().films.also { memoryStorage.setFilms(it) }
    }
}
